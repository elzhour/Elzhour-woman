      ...
      <div className="bg-card border border-border rounded-3xl p-4 mb-5 shadow-sm">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{players.length} لاعبة · {ratings.length} تقييم</p>
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === "players" && (
          <motion.div key="players" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {players.length === 0 && <div className="col-span-full text-center text-muted-foreground text-sm py-12 bg-card rounded-2xl border border-border">لا يوجد لاعبات</div>}
            {players.map((player) => {
        ...
        {activeTab === "evaluations" && (
          <motion.div key="evaluations" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-2.5">
            <SessionHeader />
            {players.length === 0 && <div className="text-center text-muted-foreground text-sm py-12 bg-card rounded-2xl border border-border">لا يوجد لاعبات</div>}
